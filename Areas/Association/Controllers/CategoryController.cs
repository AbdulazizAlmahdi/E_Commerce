...
namespace E_commerce.Areas.Association.Controllers
{
    ...
    public class CategoryController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (id == 0)
                    {
                        ...
                        categoryViewModel.category.UserId = int.Parse(HttpContext.Session.GetString("_AssUserId"));
                        if (categoryId != null)
                        {
        ...
        {
            var users = _unitOfWork.GetRepository<User>().Find(a => a.UsersId == int.Parse(HttpContext.Session.GetString("_AssUserId"))).ToList();
        ...
        public IActionResult GetCategoryData()
        {
            try
            {
                ...
                int recordsTotal = 0;
                IQueryable<Category> categoriesData = _unitOfWork.GetRepository<Category>().Include(c => c.categories).Where(c => c.UserId == int.Parse(HttpContext.Session.GetString("_AssUserId")));
