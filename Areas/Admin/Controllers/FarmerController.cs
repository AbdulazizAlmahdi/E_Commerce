    ...
    public class FarmerController : Controller
    {
        ...
        public IActionResult GetFarmer(string q)
        {
            ...
            if (!(string.IsNullOrEmpty(q) || string.IsNullOrWhiteSpace(q)))
                farmersList = _unitOfWork.GetRepository<Farmer>().GetAll().Where(i => i.Name.Contains(q)).Select(
        ...
        public IActionResult GetFarmerData()
        {
            try
            {
                ...
                int recordsTotal = 0;
                IQueryable<Farmer> farmersData = _unitOfWork.GetRepository<Farmer>().Include(d => d.Directorate.Governorate).AsQueryable();
